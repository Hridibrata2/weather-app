...
const WeekForecast = () => {
  ...
  if (!weatherData || !weatherData.forecast?.forecastday || weatherData.forecast.forecastday.length < 7) {
  ...
  const getIcon = (condition) => {
    ...
    if (cond.includes('rain')) return <FaCloudRain className='icon' size={30} />;
    if (cond.includes('cloud')) return <FaCloud className='icon' size={30} />;
    return <FaSun className='icon' size={30} />;
  }

  const [day1, day2, day3, day4, day5, day6, day7] = forecast;
